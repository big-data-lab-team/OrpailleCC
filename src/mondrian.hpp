 ...
 *   	+ rand_exponential function: A function that pick a random number between according to an exponential law. Take one double parameter, which is lambda. See https://en.wikipedia.org/wiki/Exponential_distribution for mor information about exponential distribution.
...
class MondrianForest{
	 ...
	 */
	int pick_from_distribution(double const*const probabilities, const int size) const{
		double const u = func::rand_uniform(); //Sample a random number between [0,1[
		//NOTE improve with a log search :]
		for(int i = 0; i < size-1; ++i){
			if(u >= probabilities[i] && u < probabilities[i+1])
				return i;
		}
		return size-1;
	}
	 ...
	 * [5, 15, 9] -> [0.2, 0.5, 0.3] -> [0.2, 0.7, 1.0]
	 ...
	 */
	void turn_array_into_probability(double* values, int const size, int sum = -1) const{
	...
	void extend_block(int const node_id, int const tree_id, feature_type const* features, int const label){
		...
		double const E = sum == 0 ?  -1 : func::rand_exponential(sum);
		if(E >= 0 && parent_tau + E < node.tau){//Introduce a new parent and a new sibling
			cout << "=== Introduce new nodes ===" << endl;
			turn_array_into_probability(probabilities, feature_count, sum);
			//sample features with probability proportional to e_lower[i] + e_upper[i]
			int dimension = pick_from_distribution(probabilities, feature_count);
