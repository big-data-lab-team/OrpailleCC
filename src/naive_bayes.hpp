...
class NaiveBayes{
	...
	double smoothing;
		...
		*/
	int _predict(int const* features, double* scores = nullptr){
		...
		*/
	bool _train(int const* features, int const label){
	...
	public:
		...
		*/
		NaiveBayes(int features_size[feature_count], double const smoothing=0.0){
			...
			counters = static_cast<int*>(func::malloc(label_count*sum_feature_size*sizeof(int)));
		...
		*/
		template<class T>
		int train(T* features, int const label){
			int alt_features[feature_count];
			for(int i = 0; i < feature_count; ++i)
				alt_features[i] = static_cast<int>(features[i]);
			return _train(alt_features, label);
		}
		/*
		* Train the model with one data point.
		* @param features an array of size *feature_count* that contain how many value there is for each feature.
		...
		*/
		template<class T>
		int predict(T* features, double* scores = nullptr){
			int alt_features[feature_count];
			for(int i = 0; i < feature_count; ++i)
				alt_features[i] = static_cast<int>(features[i]);
			return _predict(alt_features, scores);
		}
		/*
		* Predict the label of the given data point.
		* @param features an array of size *feature_count* that contains the value of the data point.
		...
		*/
		...
		int predict(int const* features, double* scores = nullptr){
			return _predict(features, scores);
		}
		/*
		 * Destructor
		 */
